          ...
          <table className="w-full table-fixed border-collapse" style={{ fontFamily: "'Courier New', Consolas, monospace" }}>
            ...
            <tbody>
              {filteredAndSortedReport.map((day) => { const visibleTypes = typeFilter === 'ALL' ? dailyTypes : dailyTypes.filter(item => item.key === typeFilter); return visibleTypes.map((item, index) => <tr key={`${day.date}-${item.key}`} onClick={() => onDayClick?.(day.date, item.key)} className={`group cursor-pointer transition-colors ${day.balance < 0 ? 'bg-rose-50/40 dark:bg-rose-950/20' : day.balance < 500 ? 'bg-amber-50/40 dark:bg-amber-950/20' : ''}`}>
                {index === 0 && <td rowSpan={visibleTypes.length} className="p-2 border border-slate-200 dark:border-slate-700 text-center font-bold text-slate-700 dark:text-slate-300 text-sm group-hover:bg-slate-100/80 dark:group-hover:bg-slate-800/80">{day.day}</td>}
                ...
                {index === 0 && <td rowSpan={visibleTypes.length} className={`p-2 border border-slate-200 dark:border-slate-700 text-right text-sm font-black whitespace-nowrap group-hover:bg-slate-100/80 dark:group-hover:bg-slate-800/80 ${day.balance >= 0 ? 'text-slate-800 dark:text-slate-200' : 'text-rose-700 dark:text-rose-400'}`}>{currencySymbol} {day.balance.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>}
              </tr>); })}
