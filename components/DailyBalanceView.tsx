  ...
  return (
    <div className="touch-pan-y space-y-0 transition-colors duration-300" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div className="border-b border-slate-100 dark:border-slate-800 transition-colors">
          <div className="relative flex min-h-[76px] flex-nowrap items-center gap-1 overflow-visible bg-white px-4 py-4 dark:bg-[#2f333b]">
          <div
          ...
          >
            <button aria-label="Selecionar mês e ano" onClick={() => setIsPeriodPickerOpen(value => !value)} className="shrink-0 rounded-lg p-1 text-slate-900 hover:bg-slate-100 dark:text-white dark:hover:bg-slate-700"><CalendarDays className="h-6 w-6" strokeWidth={2.5} /></button>
          ...
          <div className="mx-auto flex items-center gap-0.5">
            ...
            <span className="shrink-0 whitespace-nowrap text-2xl font-bold text-slate-800 dark:text-white">{MONTHS[parseLocalDate(dateRange.start).getMonth()]}/{String(parseLocalDate(dateRange.start).getFullYear()).slice(-2)}</span>
          ...
          <table className="w-full table-fixed border-collapse ">
            <thead className="border-b border-slate-200 dark:border-slate-700">
              <tr className="bg-white dark:bg-[#363b44]">
                <th className="w-[12%] py-3 px-2 bg-white text-left font-normal text-slate-700 dark:bg-[#363b44] dark:text-slate-100 uppercase text-xs">Dia</th>
                <th className="w-[58%] bg-white px-2 py-3 dark:bg-[#363b44]"><div className="flex justify-start"><div className="relative flex min-w-0 items-center gap-2 rounded-full border border-slate-200 px-3 py-1 dark:border-slate-700"><Grid2X2 className="h-4 w-4 shrink-0 text-slate-500 dark:text-slate-200" /><select aria-label="Filtrar por tipo" value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="min-w-0 w-full appearance-none bg-transparent pr-5 text-base font-normal text-slate-700 outline-none dark:text-slate-100"><option value="ALL">Todas</option>{dailyTypes.map(item => <option key={item.key} value={item.key}>{item.label}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 h-4 w-4 text-slate-500 dark:text-slate-200" /></div></div></th>
