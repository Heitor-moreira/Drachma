...
const CardManager: React.FC<Props> = ({ cards, onChange, currencySymbol }) => {
  ...
  return <div className="space-y-6">
    <div className="bg-white dark:bg-dark-app-surface p-6 rounded-3xl border border-slate-100 dark:border-dark-app-border">
      <div className="flex items-center justify-between mb-4"><h3 className="font-bold text-slate-800 dark:text-dark-app-text-primary">{editingId ? 'Editar cartão' : 'Cadastrar cartão'}</h3>{editingId && <button type="button" onClick={reset} className="text-xs font-bold text-slate-400">Cancelar</button>}</div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{cards.map(card => <div key={card.id} className="bg-white dark:bg-dark-app-surface p-5 rounded-3xl border border-slate-100 dark:border-dark-app-border flex items-center justify-between" style={{ borderTopColor: card.color, borderTopWidth: 4 }}><div className="flex items-center gap-3"><div className="p-3 rounded-xl text-white" style={{ backgroundColor: card.color || '#35b784' }}><CreditCardIcon size={20} /></div><div><p className="font-bold dark:text-dark-app-text-primary">{card.name}</p><p className="text-xs text-slate-400">{card.bank} · {card.type === 'CREDIT' ? 'Crédito' : 'Débito'}{card.lastFour ? ` · •••• ${card.lastFour}` : ''}</p></div></div><div className="text-right">{card.type === 'CREDIT' && <p className="font-bold dark:text-dark-app-text-primary">{currencySymbol} {(card.limit || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>}<div className="flex gap-2 justify-end mt-1"><button onClick={() => { setEditingId(card.id); edit(card); }} className="text-xs text-theme"><Pencil size={14} /></button><button onClick={() => onChange(cards.filter(c => c.id !== card.id))} className="text-xs text-rose-500"><Trash2 size={14} /></button></div></div></div>)}</div>
    <style>{`.field{width:100%;padding:.65rem .75rem;border:1px solid rgb(226 232 240);border-radius:.75rem;background:transparent;color:inherit;outline:none}.dark .field{border-color:rgb(51 65 85);background-color:rgb(30 41 59)}.field:focus{box-shadow:0 0 0 2px #35b784}`}</style>
  </div>;
};
