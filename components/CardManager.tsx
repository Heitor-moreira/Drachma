...
const emptyForm = { name: '', bank: '', brand: '', lastFour: '', color: '#2687c5', type: 'CREDIT' as CreditCard['type'], limit: '', closingDay: '1', dueDay: '10' };
...
const CardManager: React.FC<Props> = ({ cards, onChange, currencySymbol }) => {
  ...
  const edit = (card: CreditCard) => setForm({ ...emptyForm, name: card.name, bank: card.bank, brand: card.brand || '', lastFour: card.lastFour || '', color: card.color || '#2687c5', type: card.type || 'CREDIT', limit: card.limit ? String(card.limit) : '', closingDay: String(card.closingDay || 1), dueDay: String(card.dueDay || 10) });
  ...
  return <div className="space-y-6">
    <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800">
      ...
      <p className="text-[10px] text-slate-400 mt-3">Limite, fechamento e vencimento são usados apenas para cartões de crédito.</p>
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{cards.map(card => <div key={card.id} className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 flex items-center justify-between" style={{ borderTopColor: card.color, borderTopWidth: 4 }}><div className="flex items-center gap-3"><div className="p-3 rounded-xl text-white" style={{ backgroundColor: card.color || '#2687c5' }}><CreditCardIcon size={20} /></div><div><p className="font-bold dark:text-white">{card.name}</p><p className="text-xs text-slate-400">{card.bank} · {card.type === 'CREDIT' ? 'Crédito' : 'Débito'}{card.lastFour ? ` · •••• ${card.lastFour}` : ''}</p></div></div><div className="text-right">{card.type === 'CREDIT' && <p className="font-black dark:text-white">{currencySymbol} {(card.limit || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>}<div className="flex gap-2 justify-end mt-1"><button onClick={() => { setEditingId(card.id); edit(card); }} className="text-xs text-theme"><Pencil size={14} /></button><button onClick={() => onChange(cards.filter(c => c.id !== card.id))} className="text-xs text-rose-500"><Trash2 size={14} /></button></div></div></div>)}</div>
