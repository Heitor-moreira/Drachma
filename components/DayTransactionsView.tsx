...
import { getFinancialGroup, getTransactionEntryType, projectTransactions } from '../finance';
...
const DayTransactionsView: React.FC<Props> = ({ date, transactions, cards = [], currencySymbol, liteMode = false, onBack, onAdd, onEdit }) => {
  ...
  const types = [
    ...
    { key: 'SAVINGS', label: 'Economia', color: 'text-lime-600', circle: 'bg-lime-500', icon: <PiggyBank size={18} strokeWidth={3} /> },
  ...
  const selectedType = (transaction: Transaction) => {
    return visibleTypes.find(item => item.key === getTransactionEntryType(transaction)) || visibleTypes[0];
  };
