    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 transition-colors">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 h-fit transition-colors">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="text" value={newName} onChange={e => setNewName(e.target.value)} className="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none dark:text-slate-200 transition-colors" placeholder="Netflix, Disney+, VPN..." />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-1">Valor Mensal ({currencySymbol})</label>
      ...
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
        ...
        <div className="space-y-3 mb-8 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
          {subscriptions.map(s => (
            <div key={s.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${s.isActive ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700' : 'bg-slate-100/50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 opacity-60 grayscale'}`}>
              <div className="flex items-center gap-4">
                <button onClick={() => toggleStatus(s.id)} className={`p-2 rounded-xl transition-all ${s.isActive ? 'text-emerald-500 bg-emerald-50 dark:bg-emerald-500/10' : 'text-slate-400 bg-slate-100 dark:bg-slate-800'}`}>
          ...
          {subscriptions.length === 0 && (
            <div className="py-12 text-center">
               <CreditCard size={40} className="mx-auto text-slate-200 dark:text-slate-800 mb-4" />
               <p className="text-slate-400 dark:text-slate-600 font-bold italic">Nenhuma assinatura cadastrada.</p>
            </div>
          )}
        </div>
