...
const CustomDatePicker: React.FC<{ 
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 z-50 bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-100 dark:border-slate-700 p-4 w-64 animate-in fade-in zoom-in duration-200">
          <div className="flex items-center justify-between mb-4">
            <button onClick={() => setViewDate(new Date(viewDate.getFullYear(), viewDate.getMonth() - 1, 1))} className="p-1 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg dark:text-slate-300"><ChevronLeft size={16}/></button>
            <span className="text-xs font-bold text-slate-700 dark:text-slate-200">{MONTH_NAMES[viewDate.getMonth()]} {viewDate.getFullYear()}</span>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Ganhos</span>
          ...
          <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{currencySymbol} {periodData.income.toLocaleString('pt-BR')}</p>
          <div className="mt-2 text-xs text-slate-400 dark:text-slate-500">Acumulado no período</div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Despesas</span>
            <div className="p-2 bg-rose-50 dark:bg-rose-500/10 text-rose-600 dark:text-rose-500 rounded-lg"><TrendingDown size={20} /></div>
          </div>
          <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{currencySymbol} {periodData.expenses.toLocaleString('pt-BR')}</p>
          <div className="mt-2 text-xs text-slate-400 dark:text-slate-500">Saídas no período</div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">Reservado</span>
            <div className="p-2 bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-500 rounded-lg"><Landmark size={20} /></div>
          </div>
          <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{currencySymbol} {periodData.reserved.toLocaleString('pt-BR')}</p>
          <div className="mt-2 text-xs text-slate-400 dark:text-slate-500">Destinado à reserva no período</div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-slate-500 dark:text-slate-400 text-sm font-medium">% de Reserva</span>
            <div className="p-2 bg-theme/20 text-theme rounded-lg"><Percent size={20} /></div>
          </div>
          <p className="text-2xl font-bold text-slate-800 dark:text-slate-100">{periodData.reservePercentage.toFixed(1)}%</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 flex flex-col transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Movimentações</h3>
            <div className="flex items-center gap-4 bg-slate-50 dark:bg-slate-800 p-3 rounded-xl border border-slate-100 dark:border-slate-700 transition-colors">
              <CustomDatePicker label="Início" value={startDate} onChange={setStartDate} />
              <div className="h-8 w-px bg-slate-200 dark:bg-slate-700 self-end mb-1"></div>
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ bottom: 30 }}>
                ...
                <XAxis dataKey="month" axisLine={false} tickLine={false} interval={0} tick={(props) => { const { x, y, payload, index } = props; const isFirst = index === 0 || chartData[index]?.month !== chartData[index - 1]?.month; if (!isFirst) return null; return ( <text x={x} y={y + 25} fill="currentColor" className="text-slate-600 dark:text-slate-400" fontSize={11} fontWeight={700} textAnchor="start">{payload.value}</text> ); }} xAxisId="monthAxis" padding={{ left: 10, right: 10 }} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontSize: 10}} tickFormatter={(val) => `${currencySymbol} ${val >= 1000 ? (val/1000).toFixed(0)+'k' : val}`} />
                <Tooltip contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 10px 25px -5px rgb(0 0 0 / 0.1)', fontSize: '12px', backgroundColor: '#fff', color: '#1e293b' }} itemStyle={{ color: '#2687c5' }} formatter={(value: number) => [`${currencySymbol} ${value.toLocaleString('pt-BR')}`, 'Saldo']} labelFormatter={(label, payload) => { if (payload && payload.length > 0) { const item = payload[0].payload; return `${item.day} de ${item.month}`; } return label; }} labelStyle={{ fontWeight: 'bold', marginBottom: '4px', color: '#1e293b' }} />
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800 transition-colors">
          <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100 mb-6">Gastos por Categoria</h3>
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(value: number) => `${currencySymbol} ${value.toLocaleString('pt-BR')}`} contentStyle={{ borderRadius: '12px', border: 'none', backgroundColor: '#fff', color: '#1e293b' }} />
