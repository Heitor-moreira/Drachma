    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-indigo-900 dark:bg-indigo-950 p-8 rounded-[32px] text-white shadow-xl flex items-center justify-between transition-colors">
          <div>
            <p className="text-[10px] font-black text-indigo-300 dark:text-indigo-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Repeat size={14} /> Custo de Estilo de Vida</p>
            <h4 className="text-4xl font-black">{monthlyPerc.toFixed(1)}% <small className="text-sm font-bold opacity-50 uppercase">do mensal</small></h4>
        ...
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[32px] border border-slate-100 dark:border-slate-800 flex flex-col justify-center transition-colors">
          ...
          <div className="flex items-end gap-4">
            <span className="text-3xl font-black text-slate-900 dark:text-white">{currencySymbol} {annualTotal.toLocaleString('pt-BR')}</span>
            ...
            <div key={t.id} className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 flex items-center justify-between hover:border-indigo-200 dark:hover:border-indigo-500/50 transition-all group">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   ...
                   <p className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 tracking-widest">{t.category}</p>
                 </div>
               </div>
               <div className="flex items-center gap-4">
                 <span className="font-black text-slate-800 dark:text-slate-200">{currencySymbol} {t.amount.toLocaleString('pt-BR')}</span>
