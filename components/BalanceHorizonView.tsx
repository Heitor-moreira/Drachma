...
const BalanceHorizonView: React.FC<Props> = ({ transactions, dateRange, setDateRange, initialBalance, cards, currencySymbol, onBack, onAdd }) => {
  ...
  return <section className="flex h-full min-h-0 flex-col bg-white dark:bg-[#2f333b]">
    <header className="flex items-center justify-between border-b border-slate-100 px-4 py-4 dark:border-slate-800"><button onClick={onBack} aria-label="Voltar" className="rounded-xl p-2 text-slate-700 dark:text-white"><ArrowLeft size={26} /></button><h1 className="text-2xl font-bold text-slate-900 dark:text-white">Horizonte de saldos</h1><button onClick={() => onAdd(formatDate(months[activeIndex]?.month ?? anchor))} aria-label="Adicionar lançamento" className="rounded-xl p-2 text-slate-900 dark:text-white"><Plus size={30} /></button></header>
    <div ref={scrollRef} onScroll={handleScroll} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} className="min-h-0 flex-1 overflow-x-auto overflow-y-auto overscroll-x-contain overscroll-y-auto"><div className="flex min-h-full" style={{ width: `${months.length * 100 / 3}%` }}>{months.map((item, index) => <div key={formatDate(item.month)} style={{ width: `${100 / months.length}%` }} className="min-w-0 shrink-0 border-r border-slate-200 dark:border-slate-800"><div className={`border-b border-slate-200 py-2 text-center text-lg font-normal dark:border-slate-800 ${index === activeIndex ? 'bg-slate-900 text-white dark:bg-white dark:text-slate-900' : 'bg-white text-slate-900 dark:bg-slate-900 dark:text-white'}`}>{monthLabel(item.month)}</div>{item.rows.map(row => <div key={`${index}-${row.day}`} className="grid grid-cols-[38%_62%] border-b border-slate-100 text-base dark:border-slate-800"><span className="bg-slate-50 px-2 py-3 font-normal text-slate-700 dark:bg-slate-900 dark:text-slate-300">{row.day}</span><span className={`px-2 py-3 text-right font-normal ${row.balance === 0 ? 'bg-amber-100 text-slate-900 dark:bg-amber-950/40 dark:text-amber-200' : row.balance < 0 ? 'bg-rose-50 text-rose-600 dark:bg-rose-950/30' : 'bg-emerald-300 text-emerald-950 dark:bg-emerald-900/50 dark:text-emerald-100'}`}>{formatBalance(row.balance)}</span></div>)}</div>)}</div></div>
  </section>;
};
