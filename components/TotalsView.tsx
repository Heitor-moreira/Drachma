...
const TotalsView: React.FC<Props> = ({ transactions, dateRange, setDateRange, cards, currencySymbol, onOpenHorizon }) => {
  ...
  return <section className="min-h-full bg-white dark:bg-dark-app-surface">
    <header className="flex h-20 items-center justify-between border-b border-slate-100 px-4 dark:border-dark-app-border"><button aria-label="Selecionar mês" className="relative rounded-lg p-1 text-slate-900 dark:text-dark-app-text-primary"><CalendarDays size={30} /><span className="absolute inset-x-0 bottom-1 text-xs font-bold">{start.getDate()}</span></button><div className="flex items-center gap-3"><button aria-label="Mês anterior" onClick={() => moveMonth(-1)}><ChevronLeft size={25} /></button><span className="text-2xl font-bold text-slate-900 dark:text-dark-app-text-primary">{start.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' }).replace('.', '')}</span><button aria-label="Próximo mês" onClick={() => moveMonth(1)}><ChevronRight size={25} /></button></div><button aria-label="Abrir horizonte de saldos" onClick={onOpenHorizon} className="text-amber-300"><Grid2X2 size={30} /></button></header>
    <div className="border-b border-slate-100 py-24 dark:border-dark-app-border"><h2 className="px-6 text-2xl font-normal text-slate-500 dark:text-dark-app-text-secondary">Movimentações do mês</h2></div>
