...
import { getTransactionEntryType, groupTransactionsByDate, projectTransactionsWithBalance } from '../utils/finance';
...
const BalanceHorizonView: React.FC<Props> = ({ transactions, dateRange, setDateRange, initialBalance, cards, currencySymbol, onBack, onAdd, onDayClick }) => {
  ...
  const months = useMemo(() => {
    ...
    const projection = projectTransactionsWithBalance(transactions, formatDate(first), formatDate(last), cards);
    ...
    let balance = initialBalance.amount + projection.beforeStartBalanceDelta;
  ...
  return <section className="relative flex h-full min-h-0 flex-col bg-white dark:bg-dark-app-surface">
    <header className="flex h-[76px] shrink-0 items-center gap-2 border-b border-slate-200 px-4 py-4 dark:border-dark-app-border"><button onClick={onBack} aria-label="Voltar" className="rounded-lg p-1 text-slate-800 dark:text-dark-app-text-primary"><ArrowLeft className="h-6 w-6" /></button><h1 className="text-2xl font-bold text-slate-800 dark:text-dark-app-text-primary">horizonte de saldos</h1><button onClick={() => setAddMenuDate(formatDate(months[activeIndex]?.month ?? anchor))} aria-label="Adicionar lançamento" className="ml-auto rounded-xl p-2 text-slate-900 dark:text-dark-app-text-primary"><Plus size={30} /></button></header>
