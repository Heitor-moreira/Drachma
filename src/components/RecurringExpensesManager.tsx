    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-indigo-900 dark:bg-indigo-950 p-8 rounded-[32px] text-white shadow-xl flex items-center justify-between transition-colors">
          <div>
            <p className="text-xs font-bold text-indigo-300 dark:text-indigo-400 uppercase tracking-widest mb-2 flex items-center gap-2"><Repeat size={14} /> Custo de Estilo de Vida</p>
        ...
        <div className="bg-white dark:bg-dark-app-surface p-8 rounded-[32px] border border-slate-100 dark:border-dark-app-border flex flex-col justify-center transition-colors">
          <div className="flex items-center gap-3 mb-6">
            ...
            <span className="text-sm font-bold text-slate-700 dark:text-dark-app-text-secondary">Projeção Recorrente Anual</span>
          </div>
          <div className="flex items-end gap-4">
            <span className="text-3xl font-bold text-slate-900 dark:text-dark-app-text-primary">{currencySymbol} {annualTotal.toLocaleString('pt-BR')}</span>
            <span className="mb-1 text-xs font-bold text-[#35b784]">{annualPerc.toFixed(1)}% do salário anual</span>
          </div>
          <div className="mt-4 h-2 w-full bg-slate-100 dark:bg-dark-app-surface-secondary rounded-full">
      ...
      <div className="grid gap-3">
        {recurringTs.length === 0 ? (
          <div className="py-20 text-center bg-white dark:bg-dark-app-surface rounded-3xl border-2 border-dashed border-slate-100 dark:border-dark-app-border">
             <Repeat size={48} className="mx-auto text-slate-200 dark:text-dark-app-text-secondary mb-4" />
             <p className="text-slate-400 dark:text-dark-app-text-secondary font-bold">Nenhuma compra recorrente cadastrada.</p>
          </div>
        ) : (
          recurringTs.map(t => (
            <div key={t.id} className="bg-white dark:bg-dark-app-surface p-5 rounded-2xl border border-slate-100 dark:border-dark-app-border flex items-center justify-between hover:border-indigo-200 dark:hover:border-indigo-500/50 transition-all group">
               ...
               <div className="flex items-center gap-4">
                 <span className="font-bold text-slate-800 dark:text-dark-app-text-secondary">{currencySymbol} {t.amount.toLocaleString('pt-BR')}</span>
